...
const CartList = () => {
    ...
    const handleQuantityChange = async (productId, newQuantity) => {
        try {
            const accessToken = localStorage.getItem('accessToken');
            const refreshToken = localStorage.getItem('refreshToken');
            const res = await axios.put(apiUrl + `/api/v1/cart/items/${productId}`, {
                ...
                <div key={product.productId}>
                    <div style={{display:'flex', alignItems:'center', margin:'10px', marginBottom:'0px'}}>
                        ...
                        <div style={{display:'flex', alignItems:'center', maxWidth:'auto', margin:'auto'}}>
                            <DoNotDisturbOnOutlinedIcon onClick={() => handleQuantityChange(product.productId, product.quantity - 1)} />
                            <span style={{marginLeft:'5px', marginRight:'5px'}} >{product.quantity}</span>
                            <AddCircleOutlineIcon onClick={() => handleQuantityChange(product.productId, product.quantity + 1)} />
                        </div>
                    </div>
